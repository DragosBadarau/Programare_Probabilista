...
for cluster in new_clusters:
    with pm.Model() as model:
        ...
        models.append(model)
# 3
comp_models_waic = az.compare(dict(zip([str(c) for c in new_clusters], idatas)), method='BB-pseudo-BMA', ic="waic",
                              scale="deviance")
comp_models_loo = az.compare(dict(zip([str(c) for c in new_clusters], idatas)), method='BB-pseudo-BMA', ic="loo",
                             scale="deviance")
